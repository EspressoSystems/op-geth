...
func extractL1GasParams(config *params.ChainConfig, time uint64, data []byte) (l1BaseFee *big.Int, costFunc l1CostFunc, feeScalar *big.Float, err error) {
	...
	// data consists of func selector and dynamic args offset followed by 11 ABI-encoded parameters (32 bytes each)
	expectedLen := 4+32+32*11
	if len(data) < expectedLen {
		return nil, nil, nil, fmt.Errorf("expected at least %d L1 info bytes, got %d", expectedLen, len(data))
	}
	data = data[36:]                                      // trim function selector and dynamic args offset
